...
if __name__ == "__main__":
    ...
    zarr_file = zarr.group(store=zarr.N5Store(f"/path/to/save/results/sig{sig}_ref{ref}/whole_brain_mask_fused.n5"))
    shape = (D, H, W)
    chunks = (500, 512, 512)
    ...
    for z in range(0, D, 1000):
        ...
        for y in range(0, H, 1024):
            ...
            for x in range(0, W, 1024):
                ...
                mask_path = f"/path/to/blocks/cellmask/sig{sig}_ref{ref}_z{z:04}_y{y:04}_x{x:04}_cellmask.npy"
                ...
                prob = np.load(f"/path/to/blocks/cellmask/sig{sig}_ref{ref}_z{z:04}_y{y:04}_x{x:04}_cell_prob.npy")
                flow = np.load(f"/path/to/blocks/cellmask/sig{sig}_ref{ref}_z{z:04}_y{y:04}_x{x:04}_cell_flow.npy")
