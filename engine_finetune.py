...
def train(model: torch.nn.Module,
    ...
    for data_iter_step, (
        input_vol, label_mask, label_bin, label_flow
        ...
        mask_loss = F.binary_cross_entropy_with_logits(
            outputs[:, 0, :, :, :], label_bin.float(), weight=label_mask.float(), reduction="sum",
        ) / label_mask.sum().float()
        flow_loss = (
            F.mse_loss(outputs[:, 1:, :, :, :], label_flow * 5., reduction="none")
            * label_mask.unsqueeze(1)
        ).sum() / label_mask.float().sum() / 2
