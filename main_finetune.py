...
from dataset.finetune_dataset import BrainCellDataset
...
def main(args):
    ...
    dataset_train = BrainCellDataset(
        data_root=args.data_root,
        split="train",
        img_size=model.patch_embed_roi.img_size,
        ctx_size=model.patch_embed_ctx.img_size,
        alignment=model.patch_embed_ctx.patch_size,
    )
    dataset_val = BrainCellDataset(
        data_root=args.data_root,
        split="val",
        img_size=model.patch_embed_roi.img_size,
        ctx_size=model.patch_embed_ctx.img_size,
    ...
    print("Start testing ...")
    model.eval()
    os.makedirs(os.path.join(args.output_dir, "val_res"), exist_ok=True)
    with torch.no_grad():
        for i in range(misc.get_rank(), len(dataset_val), misc.get_world_size()):
            input_vol, label_vol = dataset_val[i]
            input_vol = input_vol.cuda(non_blocking=True)
            output = model(input_vol[None])[0]
            mid_depth = (input_vol.size(1) - 1) // 2
            cell_prob = torch.sigmoid(output[0, mid_depth, :, :]).cpu().numpy()
            cell_flow = output[1:, mid_depth, :, :].cpu().numpy()
            cellmask, cellp = compute_masks(
                cell_flow, cell_prob,
                min_size=0, flow_threshold=None, cellprob_threshold=.5,
            )
            torch.save({
                "pred_prob": torch.tensor(cell_prob),
                "pred_flows": torch.tensor(cell_flow),
                "instance_mask": cellmask,
            }, os.path.join(args.output_dir, "val_res", f"{i}.pt"))
