...
def remap(org_mask, stitch_mask):
    ...
    for k in area_dict:
        ...
        id1 = k >> 32 ### stitch
        if id1 != 0 and id2 != 0:
            iou_dict[id2][id1] = area_dict[k] / (area_dict[k] + area_dict.get(id2, 0) + area_dict.get(id1 << 32, 0))
    ...
    for i in np.unique(org_mask)[1:]:
        ...
        if len(iou_and_id1_list) == 0:
            ...
            overlap_id[id1].append(i) # key is the stitch, value is the orginal mask 
    ...
    for d in range(0, D, 500):
        for y in range(0, H, 512):
            for x in range(0, W, 512):
                ...
                xst+x:min(xst+x+512, xed)] = mask[d:min(d+500, zed-zst), y:min(y+512, yed-yst), x:min(x+512, xed-xst)]

if __name__ == "__main__":
    zarr_fp = zarr.open("/home/share/zlin/tatz_brain_data/240620_02_MX007-1/fused.zarr", mode="r")['0']
    ...
    zarr_file = zarr.group(store=zarr.N5Store(f"/data/share/mxia/Tatz/whole_brain_probability/whole_brain_250218/brain2/sig{sig}_ref{ref}/whole_brain_mask_fused.n5"))
    shape = (D, H, W)
    chunks = (500, 512, 512)
    ...
    for z in range(0, D, 1000):
        ...
        for y in range(0, H, 1024):
            ...
            for x in range(0, W, 1024):
                ...
                mask_path = f"/data/share/zlin/mae_cell_ctx/output_dir/blocks_250216_brain2/sig{sig}_ref{ref}_z{z:04}_y{y:04}_x{x:04}_cellmask.npy"
                ...
                loadmask(wholebrain, mask, z, z+z_sz, y, y+y_sz, x, x+x_sz)                
                
                ### load the prob and flow ###
                prob = np.load(f"/data/share/zlin/mae_cell_ctx/output_dir/blocks_250216_brain2/sig{sig}_ref{ref}_z{z:04}_y{y:04}_x{x:04}_cell_prob.npy")
                flow = np.load(f"/data/share/zlin/mae_cell_ctx/output_dir/blocks_250216_brain2/sig{sig}_ref{ref}_z{z:04}_y{y:04}_x{x:04}_cell_flow.npy")
                ...
                if x == 0:
                    ...
                    stitch_prob = np.concatenate((x_prev_prob, x_curr_prob), axis=-1)
                    if (stitch_prob < 0.5).all():
                        pass
                    else:
                        stitch_flow = np.concatenate((x_prev_flow, x_curr_flow), axis=-1)
                        stitch_mask, _ = compute_masks(stitch_flow, stitch_prob, min_size=0, flow_threshold=None, cellprob_threshold=.5, do_3D=True)
                        org_mask = wholebrain[z:z+z_sz, y:y+y_sz, x-20:x+20]
                        remap_mask = remap(org_mask, stitch_mask)
                        loadmask(wholebrain, remap_mask, z, z+z_sz, y, y+y_sz, x-20, x+20)
                ...
                if y == 0:
                    ...
                    y_prev_flow[:,:,:,x:x+x_sz] = flow[:,:z_sz,-20:,:x_sz]
                else:
                    y_curr_prob[:,:,x:x+x_sz] = prob[:z_sz,:20,:x_sz]
                    y_curr_flow[:,:,:,x:x+x_sz] = flow[:,:z_sz,:20,:x_sz]
                    y_next_prob[:,:,x:x+x_sz] = prob[:z_sz,-20:,:x_sz]
                    y_next_flow[:,:,:,x:x+x_sz] = flow[:,:z_sz,-20:,:x_sz]

                if z == 0:
                    z_prev_prob[:,y:y+y_sz,x:x+x_sz] = prob[-20:,:y_sz,:x_sz]
                    z_prev_flow[:,:,y:y+y_sz,x:x+x_sz] = flow[:,-20:,:y_sz,:x_sz]
                else:
                    z_curr_prob[:,y:y+y_sz,x:x+x_sz] = prob[:20,:y_sz,:x_sz]
                    z_curr_flow[:,:,y:y+y_sz,x:x+x_sz] = flow[:,:20,:y_sz,:x_sz]
                    z_next_prob[:,y:y+y_sz,x:x+x_sz] = prob[-20:,:y_sz,:x_sz]
                    z_next_flow[:,:,y:y+y_sz,x:x+x_sz] = flow[:,-20:,:y_sz,:x_sz]
            ...
            if y != 0:
                ...
                for stitch_x in range(0, W, 1024):
                    ...
                    stitch_prob = np.concatenate((
                                y_prev_prob[:,:,stitch_x:stitch_x+stitch_x_sz],
                                y_curr_prob[:,:,stitch_x:stitch_x+stitch_x_sz]
                                ), axis=-2)
                    ...
                    if (stitch_prob < 0.5).all():
                        ...
                        stitch_flow =  np.concatenate((
                                y_prev_flow[:,:,:,stitch_x:stitch_x+stitch_x_sz], 
                                y_curr_flow[:,:,:,stitch_x:stitch_x+stitch_x_sz]
                                ), axis=-2)

                        stitch_mask, _ = compute_masks(stitch_flow, stitch_prob, min_size=0, flow_threshold=None, cellprob_threshold=.5, do_3D=True)
                        org_mask = wholebrain[z:z+z_sz, y-20:y+20, stitch_x:stitch_x+stitch_x_sz]    
                        remap_mask = remap(org_mask, stitch_mask)
                        loadmask(wholebrain, remap_mask, z, z+z_sz, y-20, y+20, stitch_x, stitch_x+stitch_x_sz)
        ...
        if z != 0:
            ...
            for stitch_y in range(0, H, 1024):
                ...
                for stitch_x in range(0, W, 1024):
                    stitch_x_sz = min(1024, W - stitch_x)
                    stitch_prob =  np.concatenate((
                                z_prev_prob[:, stitch_y:stitch_y+stitch_y_sz, stitch_x:stitch_x+stitch_x_sz], 
                                z_curr_prob[:, stitch_y:stitch_y+stitch_y_sz, stitch_x:stitch_x+stitch_x_sz]
                                ), axis=-3)
                    if (stitch_prob < 0.5).all():
                        pass
                    else:
                        stitch_flow =  np.concatenate((
                                z_prev_flow[:, :, stitch_y:stitch_y+stitch_y_sz, stitch_x:stitch_x+stitch_x_sz], 
                                z_curr_flow[:, :, stitch_y:stitch_y+stitch_y_sz, stitch_x:stitch_x+stitch_x_sz]
                                ), axis=-3)
                            
                        stitch_mask, _ = compute_masks(stitch_flow, stitch_prob, min_size=0, flow_threshold=None, cellprob_threshold=.5, do_3D=True)
                        org_mask = wholebrain[z-20:z+20,stitch_y:stitch_y+stitch_y_sz, stitch_x:stitch_x+stitch_x_sz]
                        remap_mask = remap(org_mask, stitch_mask)
                        loadmask(wholebrain, remap_mask, z-20, z+20, stitch_y, stitch_y+stitch_y_sz, stitch_x, stitch_x+stitch_x_sz)
