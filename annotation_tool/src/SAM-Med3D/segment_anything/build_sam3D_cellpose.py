...
from .modeling  import MaskDecoder3DCellpose
...
def build_sam3D_vit_b_ori_cellpose(checkpoint=None, device='cpu'):
    return _build_sam3D_ori_cellpose(
        encoder_embed_dim=768,
        encoder_depth=12,
        encoder_num_heads=12,
        encoder_global_attn_indexes=[2, 5, 8, 11],
...
sam_model_registry3D = {
    ...
    "vit_b_ori_cellpose": build_sam3D_vit_b_ori_cellpose,
}
...
def _build_sam3D_ori_cellpose(
    ...
    prompt_embed_dim = 384
    image_size = 128
    vit_patch_size = 16 # change to 4 for 32 image size
    image_embedding_size = image_size // vit_patch_size
    sam = Sam3D(
        image_encoder=ImageEncoderViT3D(
            depth=encoder_depth,
        ...
        mask_decoder=MaskDecoder3DCellpose(
            num_multimask_outputs=3,
            transformer_dim=prompt_embed_dim,
            iou_head_depth=3,
            iou_head_hidden_dim=256,
    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f: 
            # [Change] PyTorch >=2.6
            checkpoint = torch.load(f, weights_only=False)  # PyTorch >=2.6
